...
const Component4 = ({ Data, rawData, setrawData }) => {
  const handleClick = (id) => {
    const updatedData = {
      ...rawData,
      Home: {
        ...rawData.Home,
        Component4: rawData.Home.Component4.map((item) => {
          if (item.id === id) {
            return {
              ...item,
    ...
    const newItem = updatedData.Home.Component4.find((item) => item.id === id);
    if (newItem) {
      const newCardData = {
        productPosition: "Component4",
    ...
    <section id="Edit-section">
      <div className="container">
        <div className="pransh_name d-flex gap-2 ms-3">
          <span />
          <h6 className=" mt-1">This Month</h6>
        </div>
        <div className="row m-0 m-auto">
          <div className="col-8">
            <h2 className="mt-3 mb-3">Best Selling Product</h2>
          </div>
          <div className="col-4 viewAll text-end align-content-center">
            <Link type="buttom" to="#" className="py-2 px-3">
              view all
            </Link>
          </div>
        </div>
        <div className="row mt-1 mt-lg-1 mx-0 mx-lg-2" id="selling-component">
          {Data.map((item) => (
            <div key={item.id} className="col-10 col-lg-3 m-0 m-auto my-3">
              <div className="card">
